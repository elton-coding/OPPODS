...
完整12000步，最佳step12000、validation68.17178726196289；耗时2082.162013秒，峰值分配7341425152字节。最终报告确认Encoder803392参数使用5e-5、Tx/Rx47387896参数使用1e-5，全部48,191,288参数可训练，原全局裁剪及其他协议不变。初始validation匹配V235。

固定audit精确均分 **68.30755931712964**，三噪声final为68.08672743055556、68.36299913194445、68.4729513888889；效率73.56883680555555、73.88910590277779、74.04618055555557，P10分别55.295138888888886、55.46875、55.46875。

对同预算、同解冻范围的V235，差-0.004287309，95%区间[-0.041411036,0.020260863]跨0，未检出编码器LR5倍的净收益，不宣称已证明显著更差或所有编码器学习率无效。对新冠军V242E的原offset2000配对差-0.224066312，区间[-0.258580097,-0.192286569]，不晋级不打包，也不默认组合到八专家RMS模型。

4文件哈希与最终audit已逐一核对一致；证据benchmarks/v249_encoder_lr5_audit_offset2000.json、3NPZ、v249_vs_v242e_paired.json。会话85473正常结束，PID65748/父9284已退出，不重复运行；本候选没有使用V242E确认窗口选型或拟合。以下保留预登记及历史状态。

12:50最新更新：V247完整audit68.29297453703704，未达69且未超过控制，V249队列已自动接续。GPU完整batch100两步探针通过，峰值分配7207011328、保留9911140352字节；两组参数量和学习率元数据通过门禁，证据benchmarks/v249_gpu_training_probe.json。正式12k从原V227开始，训练PID65748、父9284、会话85473；step0四项validation与V235/C一致，final68.02328529357911。探针分数不作为效果证据；未从CPU/GPU探针权重续训，当前已不是等待器。

预登记：2026-09-09 12:40。当前尚无GPU正式效果分数；分支codex/pure-neural-encoder-lr-v249继承6d599be，不改在训V242/V247或排队V248的绑定依赖。

12:43执行：全套测试通过、10项跳过，新增5项测试及Ruff通过；CPU真实父模型探针已留档。代码和预登记18b8736已推送。等待器PID9284/会话85473已启动，等待V247完整audit，不占第三项GPU训练；GPU batch100探针及正式12k尚未运行，不重复启动。
