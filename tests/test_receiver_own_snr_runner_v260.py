...
import run_pure_neural_receiver_v260 as runner


def control_report():
    return json.loads((ROOT / "benchmarks/v250_eight_rms_72k_audit_offset2000.json").read_text())["training_report"]
...
def test_only_design_and_output_change_in_original_full72k_entry():
...
def test_reports_require_all_validation_points_and_full_gpu_probe():
    report = control_report()  # Schema fixture only; not V260 performance.
...
@pytest.mark.parametrize("key,value", [
    ("requested_steps", 36000), ("score_temperature", .25), ("score_bce_weight", .01),
...
def test_real_cpu_proof_and_synthetic_gpu_schema(monkeypatch):
    ...
    gpu.update(device="cuda", batch_size=100, transmitter_route_counts=[13]*4+[12]*4,
               receiver_route_counts=[26,26,13,13,12,12,12,86], gpu_memory_fraction=.4,
               gpu_peak_allocated_bytes=1, identity=None, serialization=None)
...
@pytest.mark.parametrize("failure", ["hash", "grad", "logits", "hard", "rng", "serialization", "route", "encoder", "finite"])
def test_runtime_proof_rejects_missing_or_wrong_invariants(monkeypatch, failure):
    ...
    if failure == "hash":
        ...
        record["identity"]["gradient_maximum_differences"]["receiver_parent_sum"] = .1
    elif failure == "logits":
        record["identity"]["batch_logit_maximum_difference"] = .001
    elif failure == "hard":
        record["identity"]["hard_decision_disagreements"] = 1
    elif failure == "rng":
        record["identity"]["channel_rng_equal"] = False
    elif failure == "serialization":
        record["serialization"] = None
    elif failure == "route":
        record["receiver_route_counts"][0] = 0
...
def test_missing_or_partial_predecessor_does_not_release_queue(tmp_path, monkeypatch, contents):
    ...
    with pytest.raises(TimeoutError, match="V258 incomplete"):
