...
import pytest
import run_shared_lr_v269 as v
...
def test_only_lr_and_output():
    for probe in (False, True):
        ...
        for flag in ('--learning-rate', '--output-dir'):
            ...
            cmd[i] = base[i]
        assert cmd == base
    assert v.CONTROL == 'v268_shared8_rms_lr2e5_72k'


def test_guard():
    for probe, path in ((True, 'artifacts/resource_probe/v268/shared_lr/training_report.json'),
                        (False, 'artifacts/pure_neural_v268/eight/lr2e5_steps72000/training_report.json')):
        report = copy.deepcopy(v.read(v.ROOT / path))
        report['learning_rate'] = 3e-5
        v.require_result(report, probe)
        for key, value in [('learning_rate', 2e-5), ('learning_rate_schedule', {}),
