"""Package and CPU-check frozen V257; deliberately not a release or score claim."""
...
from run_frozen_confirmation_v257 import PLAN_PATH, make_plan
...
DIRECTORY = ROOT / "artifacts/pure_neural_v257/eight/shared8_steps72000"
ARCHIVE = ROOT / "artifacts/pure_neural_v257/candidate_package/v257_shared_prefix8_candidate_not_released.zip"
OUTPUT = ROOT / "benchmarks/v257_candidate_package_verification.json"
...
def main():
    ...
    if files != registered["sources"][1]["files"]:
        raise RuntimeError("candidate differs from frozen confirmation source")
    sources = fingerprints([ROOT / name for name in (
        "scripts/verify_candidate_package_v257.py", "scripts/build_submission.py",
        "scripts/probe_pure_neural_shared_v257.py", "scripts/evaluate_submission.py",
        "scripts/train_pure_neural_snr_experts.py", "benchmarks/v257_confirmation_plan.json")])
