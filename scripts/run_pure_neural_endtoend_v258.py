"""V258: only unfreeze Encoder under the eight-expert RMS full72k protocol."""
...
from probe_pure_neural_endtoend_v258 import COMPONENTS, CPU_PROOF, GPU_PROOF, source_paths
...
from run_pure_neural_rms_temperature_v256 import require_result as require_temperature_result
from run_storage_factorial_v254 import verify_bound_inputs, wait_gpu_slot, write_new

CONTROL = "v250_eight_rms_72k"
DEPENDENCY = "v256_eight_rms_temperature025_72k"
LABEL = "v258_eight_rms_endtoend_72k"
OUTPUT = "artifacts/pure_neural_v258/eight/endtoend_steps72000"
PROBE = "artifacts/resource_probe/v258/eight_endtoend"
DEPENDENCY_DIR = "artifacts/pure_neural_v256/eight/temperature025_steps72000"
...
def require_result(report, *, probe=False):
    if (report.get("train_components") != COMPONENTS or report.get("trainable_parameters") != 190354976
            or report.get("variable_payload") is not False
            or any(k in report for k in ("learning_rate_schedule", "encoder_learning_rate"))):
        raise ValueError("V258 needs original architecture, all components, single constant LR")
    if not probe:
        require_report({**report, "train_components": ["transmitter", "receiver"]}, 72000)
        return
...
def require_runtime(record, device):
    ...
    expected = {"passed": True, "device": device, "weights_saved": False,
                ...
                "trainable_parameters": 190354976, "gpu_memory_fraction": .4 if cuda else None}
    rows = record.get("steps", [])
    count = record.get("trainable_parameter_tensors", 0)
    if (any(record.get(k) != v for k, v in expected.items()) or count != 1300
            or [r["step"] for r in rows] != [1, 2]
            or any(r.get("adam_parameter_states") != count
                   or not math.isfinite(r.get("encoder_maximum_parameter_change_from_initial", 0))
                   or r.get("encoder_maximum_parameter_change_from_initial", 0) <= 0
                   for r in rows)):
        raise ValueError("incomplete end-to-end state/update proof")
    for row in rows:
        components = row.get("component_gradients", {})
        if (set(components) != set(COMPONENTS)
                or components["encoder"].get("parameters") != 803392
                or sum(v.get("parameters", 0) for v in components.values()) != 190354976
                or sum(v.get("gradient_tensors", 0) for v in components.values()) != count
                or any(not math.isfinite(v.get("gradient_l1", 0)) or v.get("gradient_l1", 0) <= 0
                       for v in components.values())):
            raise ValueError("Encoder/transceiver gradient proof incomplete")
    if cuda:
        if record.get("gpu_peak_allocated_bytes", 0) <= 0:
            raise ValueError("missing CUDA allocation evidence")
    else:
        identity = record.get("identity") or {}
        if (identity.get("logit_maximum_difference") != 0 or identity.get("loss_difference") != 0
                or identity.get("rng_equal") is not True or identity.get("frozen_encoder_has_no_gradients") is not True
                or not math.isfinite(identity.get("transceiver_gradient_maximum_difference_before_clip", float("inf")))
                or identity.get("transceiver_gradient_maximum_difference_before_clip", float("inf")) > 1e-5):
            raise ValueError("CPU initial-function or gradient identity failed")
        ...
        if path.exists():
            try:
                ...
                return {"input_sha256": fingerprints(paths), "exact_mean_final": audit["exact_mean_final"],
                        "resource_dependency_only": True, "v258_control": CONTROL}
        if time.monotonic() >= deadline:
            raise TimeoutError("V256 incomplete; inspect original process, no restart")
...
def main():
    ...
    reserved = [output, probe, output.parent / "execution_plan.json", output.parent / "execution.lock",
                ...
                ROOT / "benchmarks/v258_gpu_training_probe.json", ROOT / "benchmarks/v258_initial_validation_check.json",
    ...
    if any(p.exists() for p in reserved):
        raise FileExistsError("V258 output exists; no implicit restart/resume/overwrite")
    ...
    prior_path = ROOT / "artifacts/pure_neural_v257/eight/execution_plan.json"
    ...
    paths += [ROOT / "scripts/run_pure_neural_endtoend_v258.py"]
    plan = {"label": LABEL, "control": CONTROL, "dependency": DEPENDENCY,
            ...
            "single_factor": "Encoder frozen to trainable; eight-expert RMS72k and unified constant Adam1e-5 unchanged",
            "initialization": "original V227; fresh Adam, no probe/control/dependency weights reused",
            "trainable_parameters": 190354976, "automatic_promotion": False}
    ...
    try:
        ...
        wait_gpu_slot()
        subprocess.run([sys.executable, "-u", "scripts/probe_pure_neural_endtoend_v258.py", "--device", "cuda"],
                       cwd=ROOT, check=True)
        ...
        write_new(ROOT / "benchmarks/v258_gpu_training_probe.json", {"purpose": "initial/runtime only, not score",
        ...
        write_new(ROOT / "benchmarks/v258_initial_validation_check.json", {"matched": True,
