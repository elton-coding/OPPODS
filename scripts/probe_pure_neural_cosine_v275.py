...
from train_pure_neural_cosine_v275 import RateRecorder, require_schedule

EVIDENCE = "benchmarks/v275_cosine_cpu_proof.json"


def source_paths():
    return [ROOT / p for p in ("scripts/probe_pure_neural_cosine_v275.py",
            "scripts/train_pure_neural_cosine_v275.py", "scripts/train_pure_neural_rms_v239.py",
            "scripts/train_pure_neural_snr_experts.py", "scripts/run_pure_neural_lr_v237.py")]


def main():
    evidence = ROOT / EVIDENCE
    directory = ROOT / "artifacts/resource_probe/v275/cosine_cpu"
    ...
    with (directory / "execution_plan.json").open("x", encoding="utf-8") as stream:
        json.dump({"input_sha256": before, "updates": 144000, "constant_prefix": 72000,
                   "seed": 275001, "parameters": 4, "device": "cpu", "model_score_evaluated": False}, stream, indent=2)
    ...
    optimizers = [torch.optim.Adam([p], lr=3e-5) for p in (actual, reference, constant)]
    recorder = RateRecorder(optimizers[0])
    generator = torch.Generator().manual_seed(275001)
    gradients = torch.randn(144000, 4, generator=generator)
    ...
    for update, gradient in enumerate(gradients, 1):
        ...
        optimizers[1].param_groups[0]["lr"] = (3e-5 if update <= 72000 else
            3e-5 * (1./3. + (1.-1./3.) * .5 * (1. + math.cos(math.pi * (update - 72000) / 72000))))
        ...
        if update <= 72000 and (not torch.equal(actual, constant) or any(not torch.equal(
    ...
    result = {"passed": True, "device": "cpu", "parameters": 4, "updates_checked": 144000,
              "constant_prefix_parameter_and_adam_state_exact_updates": 72000,
              "manual_schedule_parameter_and_adam_state_exact_updates": 144000,
