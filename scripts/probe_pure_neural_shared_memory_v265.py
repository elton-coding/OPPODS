...
import torch
from probe_pure_neural_shared_v265 import DESIGN, EVIDENCE, MAPPING, PARENT, assert_aliases, source_paths
...
OUTPUT = ROOT / "benchmarks/v265_full_state_gpu_probe.json"
PLAN = ROOT / "artifacts/resource_probe/v265/full_state/execution_plan.json"
MEMORY_FRACTION = .4
...
def main():
    if OUTPUT.exists() or PLAN.exists():
        raise FileExistsError("V265 full-state GPU probe already attempted; inspect, do not overwrite")
    ...
    paths = source_paths() + [ROOT / name for name in (
        EVIDENCE, "scripts/probe_pure_neural_shared_memory_v265.py",
    ...
    for step in (1, 2):
        ...
        if len(gradients) != 598 or not all(torch.isfinite(g).all() for g in gradients):
    ...
    if len(parameters) != 598 or sum(p.numel() for p in parameters) != 83767368:
    ...
    result = {"purpose": "all-eight-route allocation and optimizer runtime; not formal training or performance",
              ...
              "steps": records, "trainable_parameters": 83767368, "optimizer_state_bytes": state_bytes,
