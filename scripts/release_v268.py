"""Publish verified V268 files locally; preserve immutable V257 and V268 archives."""
import hashlib
import shutil
...
def main():
    root = source.ROOT
    out = root / 'benchmarks/v268_release_verification.json'
    fixed = root / 'artifacts/FATE_MIMO_submission_pure_neural_v268_shared_lr2e5_rms_72k_official.zip'
    ...
    evidence = source.read(root / 'benchmarks/v268_candidate_package_verification.json')
    ...
    old = source.read(root / 'benchmarks/v257_release_verification.json')
    assert sha(root / old['archive']) == old['archive_sha256']
    assert sha(root / old['canonical_archive']) == old['archive_sha256']
    ...
    assert sha(root / old['archive']) == old['archive_sha256']
    source.verify_bound_inputs(source.read(frozen.PLAN))
    source.write_new(out, {'version': 'V268', 'archive': str(fixed.relative_to(root)),
                          ...
                          'full_cpu_pytest_session': 7646, 'online_confirmation': False,
                          'whole_project_blindness_certified': False,
                          'organizer_inference_time_certified': False})
    print('V268 local release verified; Git publication still required')
