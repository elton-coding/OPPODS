"""V275: unchanged RMS/Adam training, constant72k then cosine72k to one third LR."""
...
def learning_rate(update):
    if not isinstance(update, int) or not 1 <= update <= TOTAL_STEPS:
        raise ValueError("schedule uses successful optimizer updates numbered1..144000")
...
def schedule_definition():
    return {"kind": "constant_then_cosine", "hold_updates": HOLD_STEPS, "total_updates": TOTAL_STEPS,
            ...
            "probe_uses_full144000_schedule_not_two_step_decay": True}
...
class RateRecorder:
    ...
    def __init__(self, optimizer):
        if len(optimizer.param_groups) != 1 or optimizer.param_groups[0]["lr"] != INITIAL_LR:
            raise ValueError("V275 requires one unchanged Adam group at initial3e-5")
        self.completed = 0
        self.pending = None
        self.trace = []
...
def main():
    ...
    if (args.stage != "calibrate" or args.loss_kind != "rms_score" or args.steps not in (2, TOTAL_STEPS)
        ...
        raise ValueError("V275 supports only the registered full144k run or initial two-step probe")
    if args.output_dir.exists():
        raise FileExistsError("V275 refuses implicit checkpoint/optimizer resume")
