"""V269: only increase V268 RMS constant learning rate2e-5 to3e-5; fresh full72k."""
...
from run_shared_lr_v268 import command as baseline_command
from run_shared_lr_v268 import require_result as baseline_guard

CONTROL = "v268_shared8_rms_lr2e5_72k"
CONTROL_DIR = "artifacts/pure_neural_v268/eight/lr2e5_steps72000"
LABEL = "v269_shared8_rms_lr3e5_72k"
BASE = ROOT / "artifacts/pure_neural_v269/eight"
OUTPUT = BASE / "lr3e5_steps72000"
PROBE = ROOT / "artifacts/resource_probe/v269/shared_lr"
PLAN = BASE / "execution_plan.json"
DECISION = ROOT / "benchmarks/v269_selection_decision.json"


def command(probe=False):
    cmd = baseline_command(probe=probe)
    cmd[cmd.index("--learning-rate")+1] = "3e-5"
    cmd[cmd.index("--output-dir")+1] = str(PROBE if probe else OUTPUT)
    return cmd


def require_result(report, probe=False):
    if report.get("learning_rate") != 3e-5 or "learning_rate_schedule" in report:
        raise ValueError("V269 requires constant3e-5 without schedule")
    baseline_guard({**report, "learning_rate": 2e-5}, probe=probe)
...
def main():
    ...
    probe_path = ROOT / "benchmarks/v269_gpu_probe.json"
    prefix_path = ROOT / "benchmarks/v269_initial_check.json"
    ...
    if any(p.exists() for p in reserved):
        raise FileExistsError("V269 reserved outputs exist; no implicit restart")
    previous_path = ROOT / "artifacts/pure_neural_v268/eight/execution_plan.json"
    ...
    paths += [ROOT / f"benchmarks/{CONTROL}_audit_offset2000.json",
              ROOT / "benchmarks/v268_selection_decision.json"]
    paths += score_paths(ROOT / "benchmarks", CONTROL, SEEDS, 2000)
    paths += [ROOT / p for p in (
        "scripts/run_shared_lr_v269.py", "tests/test_shared_lr_v269.py",
        "docs/experiments/shared-lr-v269.md")]
    plan = {"input_sha256": fingerprints(paths), "dataset": data_record(), "command": command(),
            ...
            "single_factor": "constant learning rate2e-5 to3e-5; V268 unchanged elsewhere",
