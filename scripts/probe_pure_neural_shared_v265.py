"""CPU-only identity, tied-gradient, optimizer and serialization proof for V265."""
...
DESIGN = "research/pure_neural_v265/modelDesign.py"
...
EVIDENCE = "benchmarks/v265_cpu_shared_prefix_probe.json"


def source_paths():
    names = [DESIGN, CONTROL_DESIGN, "scripts/probe_pure_neural_shared_v265.py",
...
def canonical_name(component, expert_index, name):
    if shared_name(component, name):
        expert_index = leader_index(component, expert_index)
    return f"{component}.experts.{expert_index}.{name}"


def leader_index(component, index):
    return index if component == "transmitter" and index < 2 else (0 if index < 2 else 2)
...
def assert_aliases(link):
    ...
    for component in ("transmitter", "receiver"):
        ...
        for index, expert in enumerate(experts):
            leader = experts[leader_index(component, index)]
            ...
            for other_index, other in enumerate(experts):
                if leader_index(component, other_index) != leader_index(component, index):
...
def make_link(design):
    ...
    if design == DESIGN and list(module.SHARED_PARENT_MAP) != MAPPING:
        raise ValueError("V265 sharing groups require the original V227 parent map")
