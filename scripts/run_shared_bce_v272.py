"""V272: only halve V269 raw BCE weight0.05 to0.025; fresh full72k."""
...
LABEL = "v272_shared8_rms_bce025_72k"
BASE = ROOT / "artifacts/pure_neural_v272/eight"
OUTPUT = BASE / "bce025_steps72000"
PROBE = ROOT / "artifacts/resource_probe/v272/shared_bce"
PLAN = BASE / "execution_plan.json"
DECISION = ROOT / "benchmarks/v272_selection_decision.json"


def command(probe=False):
    cmd = baseline_command(probe=probe)
    cmd[cmd.index("--score-bce-weight")+1] = "0.025"
    cmd[cmd.index("--output-dir")+1] = str(PROBE if probe else OUTPUT)
    return cmd


def require_result(report, probe=False):
    if report.get("score_bce_weight") != 0.025:
        raise ValueError("V272 requires BCE0.025")
    baseline_guard({**report, "score_bce_weight": 0.05}, probe=probe)
...
def main():
    ...
    probe_path = ROOT / "benchmarks/v272_gpu_probe.json"
    prefix_path = ROOT / "benchmarks/v272_initial_check.json"
    ...
    if any(p.exists() for p in reserved):
        raise FileExistsError("V272 reserved outputs exist; no implicit restart")
    previous_path = ROOT / "artifacts/pure_neural_v271/eight/execution_plan.json"
    ...
    paths += [ROOT / p for p in (
        "scripts/run_shared_bce_v272.py", "tests/test_shared_bce_v272.py",
        "docs/experiments/shared-bce-v272.md")]
    plan = {"input_sha256": fingerprints(paths), "dataset": data_record(), "command": command(),
            ...
            "single_factor": "raw BCE weight0.05 to0.025; V269 unchanged elsewhere",
