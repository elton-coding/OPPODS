"""V260: receiver own-SNR routing with unchanged eight-expert RMS full72k protocol."""
...
from probe_pure_neural_receiver_v260 import CPU_PROOF, DESIGN, GPU_PROOF, source_paths
from run_pure_neural_endtoend_v258 import require_result as require_dependency_result
...
DEPENDENCY = "v258_eight_rms_endtoend_72k"
LABEL = "v260_eight_rms_receiver_own_72k"
OUTPUT = "artifacts/pure_neural_v260/eight/receiver_own_steps72000"
PROBE = "artifacts/resource_probe/v260/eight_receiver_own"
DEPENDENCY_DIR = "artifacts/pure_neural_v258/eight/endtoend_steps72000"
...
def require_result(report, *, probe=False):
    if (report.get("train_components") != ["transmitter", "receiver"]
        ...
        raise ValueError("V260 changes only Receiver routing, not budget/architecture size/objective")
    if not probe:
...
def require_runtime(record, device):
    ...
    expected = {"passed": True, "device": device, "weights_saved": False,
                ...
                "transmitter_route_counts": [13]*4+[12]*4 if cuda else [1]*8,
                "receiver_route_counts": [26,26,13,13,12,12,12,86] if cuda else [2,2,1,1,1,1,1,7],
                "encoder_frozen_and_unchanged": True, "gpu_memory_fraction": .4 if cuda else None}
    ...
    if (any(record.get(k) != v for k, v in expected.items())
        ...
        raise ValueError("incomplete all-route receiver-routing runtime proof")
    if cuda:
        if record.get("gpu_peak_allocated_bytes", 0) <= 0:
            raise ValueError("missing GPU peak")
    else:
        identity = record.get("identity") or {}
        gradients = identity.get("gradient_maximum_differences") or {}
        serialization = record.get("serialization") or {}
        if (identity.get("single_logit_maximum_difference") != 0
                or identity.get("hard_decision_disagreements") != 0
                or identity.get("construction_rng_equal") is not True or identity.get("channel_rng_equal") is not True
                or set(gradients) != {"transmitter", "receiver_parent_sum"}
                or serialization.get("all_parameters_and_outputs_equal") is not True
                or serialization.get("bytes", 0) <= 0):
            raise ValueError("missing initial identity/grouped-gradient/serialization proof")
        values = [(identity.get("batch_logit_maximum_difference", float("inf")), 2e-4),
                  (identity.get("loss_difference", float("inf")), 1e-6)]
        values += [(v, 1e-5) for v in gradients.values()]
        if any(not math.isfinite(v) or abs(v) > limit for v, limit in values):
            raise ValueError("initial function/grouped-gradient differences exceed registered bounds")
...
def require_design(directory, plan):
    if fingerprint(directory)["modelDesign.py"]["sha256"] != plan["input_sha256"][DESIGN]:
        raise ValueError("trained output does not use the frozen receiver-routing design")
        ...
        if path.exists():
            try:
                ...
                if (fingerprint(ROOT / DEPENDENCY_DIR) != audit["files"]
                    ...
                    raise RuntimeError("completed V258 dependency changed")
                ...
                return {"input_sha256": fingerprints(paths), "exact_mean_final": audit["exact_mean_final"],
                        "resource_dependency_only": True, "v260_control": CONTROL}
        if time.monotonic() >= deadline:
            raise TimeoutError("V258 incomplete; inspect original process, no restart")
...
def main():
    ...
    reserved = [output, probe, output.parent / "execution_plan.json", output.parent / "execution.lock",
                ...
                ROOT / "benchmarks/v260_gpu_training_probe.json", ROOT / "benchmarks/v260_initial_validation_check.json",
    ...
    if any(p.exists() for p in reserved):
        raise FileExistsError("V260 output exists; no implicit restart/resume/overwrite")
    ...
    prior_path = ROOT / "artifacts/pure_neural_v259/eight/execution_plan.json"
    ...
    paths += [ROOT / "scripts/run_pure_neural_receiver_v260.py"]
    plan = {"label": LABEL, "control": CONTROL, "dependency": DEPENDENCY,
            ...
            "single_factor": "Only Rx route: preserve parent group then own SNR; unchanged Tx routes, capacity and RMS72k",
    ...
    try:
        ...
        wait_gpu_slot()
        subprocess.run([sys.executable, "-u", "scripts/probe_pure_neural_receiver_v260.py", "--device", "cuda"],
                       cwd=ROOT, check=True)
        ...
        write_new(ROOT / "benchmarks/v260_gpu_training_probe.json", {"purpose": "initial/runtime only, not score",
        ...
        write_new(ROOT / "benchmarks/v260_initial_validation_check.json", {"matched": True,
