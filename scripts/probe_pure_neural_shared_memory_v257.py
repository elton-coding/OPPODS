"""Full batch100/all8-routes/two-Adam-step GPU allocation probe; no score claim."""
...
import torch
from probe_pure_neural_shared_v257 import DESIGN, EVIDENCE, MAPPING, PARENT, assert_aliases, source_paths
...
OUTPUT = ROOT / "benchmarks/v257_full_state_gpu_probe.json"
PLAN = ROOT / "artifacts/resource_probe/v257/full_state/execution_plan.json"
MEMORY_FRACTION = .4


def forced_snr(device):
    routes = torch.arange(100, device=device) % 8
    minimum = -17.5 + 5. * routes.float()
...
def main():
    if OUTPUT.exists() or PLAN.exists():
        raise FileExistsError("V257 full-state GPU probe already attempted; inspect, do not overwrite")
    cpu = json.loads((ROOT / EVIDENCE).read_text(encoding="utf-8"))
    if cpu.get("passed") is not True or fingerprints(source_paths()) != cpu["input_sha256"]:
        raise ValueError("CPU proof missing or stale")
    active = check_training_slot()
    paths = source_paths() + [ROOT / name for name in (
        EVIDENCE, "scripts/probe_pure_neural_shared_memory_v257.py",
    ...
    with PLAN.open("x", encoding="utf-8") as stream:
        json.dump({"input_sha256": before, "other_registered_training_pids": active, "pid": os.getpid(),
                   "batch_size": 100, "adam_steps": 2, "all_routes_forced": 8,
    ...
    counts = torch.bincount(module._expert_indices(snr.amin(dim=1)), minlength=8).cpu().tolist()
    if counts != [13] * 4 + [12] * 4:
        raise ValueError("GPU probe does not cover all eight routes")
    records = []
    for step in (1, 2):
        ...
        if len(gradients) != 528 or not all(torch.isfinite(g).all() for g in gradients):
            raise RuntimeError("missing or nonfinite gradient: all eight experts must participate")
    ...
    if len(parameters) != 528 or sum(p.numel() for p in parameters) != 72744448:
    ...
    result = {"purpose": "all-eight-route allocation and optimizer runtime; not formal training or performance",
              ...
              "steps": records, "trainable_parameters": 72744448, "optimizer_state_bytes": state_bytes,
              ...
              "shared_parameter_alias_checks_after_cuda": alias_checks}
