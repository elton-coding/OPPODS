"""Publish verified V269 files locally; preserve immutable V268 and V269 archives."""
import hashlib
import shutil

import run_frozen_confirmation_v269 as frozen
import run_shared_lr_v269 as source
...
def main():
    root = source.ROOT
    out = root / 'benchmarks/v269_release_verification.json'
    fixed = root / 'artifacts/FATE_MIMO_submission_pure_neural_v269_shared_lr3e5_rms_72k_official.zip'
    ...
    evidence = source.read(root / 'benchmarks/v269_candidate_package_verification.json')
    ...
    old = source.read(root / 'benchmarks/v268_release_verification.json')
    ...
    assert sha(root / old['archive']) == old['archive_sha256']
    source.verify_bound_inputs(source.read(frozen.PLAN))
    source.write_new(out, {'version': 'V269', 'archive': str(fixed.relative_to(root)),
                          ...
                          'full_cpu_pytest_session': 95905, 'online_confirmation': False,
                          'whole_project_blindness_certified': False,
                          'organizer_inference_time_certified': False})
    print('V269 local release verified; Git publication still required')
