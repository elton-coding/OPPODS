...
ROOT = source.ROOT
SEEDS = [82701, 82702, 82703]
OFFSET = 8000
LABELS = ['v269_confirm_v273', 'v273_confirm']
DIRS = [ROOT / source.CONTROL_DIR, source.OUTPUT]
PLAN = ROOT / 'benchmarks/v273_confirmation_plan.json'
RESULT = ROOT / 'benchmarks/v273_confirmation_decision.json'
FAILURE = ROOT / 'benchmarks/v273_confirmation_failure.json'
...
def main():
    ...
    binding += [ROOT / p for p in ('scripts/run_frozen_confirmation_v273.py',
                'tests/test_frozen_confirmation_v273.py', 'docs/experiments/frozen-confirmation-v273.md',
                'benchmarks/v273_confirmation_channel_inventory.json')]
