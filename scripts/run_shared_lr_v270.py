"""V270: only increase V269 RMS constant learning rate3e-5 to4e-5; fresh full72k."""
...
LABEL = "v270_shared8_rms_lr4e5_72k"
BASE = ROOT / "artifacts/pure_neural_v270/eight"
OUTPUT = BASE / "lr4e5_steps72000"
PROBE = ROOT / "artifacts/resource_probe/v270/shared_lr"
PLAN = BASE / "execution_plan.json"
DECISION = ROOT / "benchmarks/v270_selection_decision.json"


def command(probe=False):
    cmd = baseline_command(probe=probe)
    cmd[cmd.index("--learning-rate")+1] = "4e-5"
    cmd[cmd.index("--output-dir")+1] = str(PROBE if probe else OUTPUT)
    return cmd


def require_result(report, probe=False):
    if report.get("learning_rate") != 4e-5 or "learning_rate_schedule" in report:
        raise ValueError("V270 requires constant4e-5 without schedule")
...
def main():
    ...
    probe_path = ROOT / "benchmarks/v270_gpu_probe.json"
    prefix_path = ROOT / "benchmarks/v270_initial_check.json"
    ...
    if any(p.exists() for p in reserved):
        raise FileExistsError("V270 reserved outputs exist; no implicit restart")
    previous_path = ROOT / "artifacts/pure_neural_v269/eight/execution_plan.json"
    ...
    paths += [ROOT / p for p in (
        "scripts/run_shared_lr_v270.py", "tests/test_shared_lr_v270.py",
        "docs/experiments/shared-lr-v270.md")]
    plan = {"input_sha256": fingerprints(paths), "dataset": data_record(), "command": command(),
            ...
            "single_factor": "constant learning rate3e-5 to4e-5; V269 unchanged elsewhere",
