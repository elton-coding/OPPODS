...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate two reserved pilots per frequency group")
    ...
    parser.add_argument("--pilot-amplitude", type=float, default=2**0.5)
...
def main() -> None:
    ...
    link = ReservedPilotMUMIMOLink(args.pilot_amplitude).to(device)
    ...
    for start in range(0, count, args.batch_size):
        ...
        llr, identity = link(channel, bits, snr, generator=generator)
        score_batches.append(
            torch.stack([per_sample_score(bits[:, user], llr[:, user]) for user in range(2)], dim=1).cpu().numpy()
        )
        one_bit_batches.append(
            torch.stack([per_sample_score(bits[:, user], llr[:, user, :1]) for user in range(2)], dim=1).cpu().numpy()
        )
        identity_batches.append(identity.cpu().numpy())
    scores = np.concatenate(score_batches)
    one_bit_scores = np.concatenate(one_bit_batches)
    summary = summarize_scores(scores)
    best_rule = None
    for threshold in np.arange(-20.0, 5.01, 0.5):
        dynamic = np.where(snr_np < threshold, one_bit_scores, scores)
    ...
    print(
        json.dumps(
            {
                "samples": count,
                "pilot_amplitude": args.pilot_amplitude,
                "efficiency": summary.efficiency,
                "fairness": summary.fairness,
                "final": summary.final,
                "best_rule": best_rule,
                "pilot_identity_accuracy": float(np.concatenate(identity_batches).mean()),
                "snr_slices": {
                    f"{low}:{low + 5}": {
                        "mean": float(scores[(snr_np >= low) & (snr_np < low + 5)].mean()),
                        "p10": float(np.percentile(scores[(snr_np >= low) & (snr_np < low + 5)], 10)),
                    }
