...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V149 延续 V147 的分段软干扰抵消专家，并在 `[10.5,15.75)` dB 把数据增益细化后验温度从 `0.5` 调整为 `0.6`。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.381717`，公平 P10 `50.694444`，总分 **`63.075535`**。三个固定 seed 的平均总分为 **`63.213499`**，相对 V147 提升 `0.000446`；冻结后 seed2098/2099/2100 三项盲测分别提升 `0.000987/0.000608/0.000911`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V149](docs/solutions/FATE-MIMO_v149.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
