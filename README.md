...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V156 延续 V153 的高中间 SNR 扩展门控，并新增 `[-3.95,-3.4) dB` 内部稳健子区间：把 924→1056 bit 扩展置信度门槛提高到 `0.55`。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.382064`，公平 P10 `50.694444`，总分 **`63.075778`**。三个固定 seed 的平均总分为 **`63.214026`**，相对 V153 提升 `0.000056`；冻结后 seed2106/2107/2108 三项新盲测分别提升 `0/+0.000122/0`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V156](docs/solutions/FATE-MIMO_v156.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
