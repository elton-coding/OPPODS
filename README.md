...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V134 将 Receiver 协方差加载细分为三档：`[-20,8.75) dB` 使用 `4.0`，`[8.75,11.65) dB` 使用 `5.0`，`[11.65,20) dB` 使用 `7.0`。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.352507`，公平 P10 `50.694444`，总分 **`63.055088`**。三个固定 seed 的平均总分为 **`63.195655`**，相对 V133 提升 `0.001595`；冻结后 seed2056/2057/2058 三项盲测分别提升 `0.002977/0.001306/0.003965`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V134](docs/solutions/FATE-MIMO_v134.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
