...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 UE SNR 路由的 Receiver 物理参数专家。V118 在五个经过 2000×3 配对验证的窄 SNR 区间中独立调整干扰抵消或软估计温度，其他区间保持 V117 参数。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.245530`，公平 P10 `50.607639`，总分 **`62.954162`**。三个固定 seed 的平均总分为 **`63.080436`**，相对 V117 提升 `0.002977`。这些是本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V118](docs/solutions/FATE-MIMO_v118.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
