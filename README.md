...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V152 延续 V149 的数据增益温度专家，并把 `[-5,-2.5) dB` 的 924→1056 bit 扩展置信度门槛从 `0.260` 提高到 `0.296287667`，减少低一致性扩展输出。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.381717`，公平 P10 `50.694444`，总分 **`63.075535`**。三个固定 seed 的平均总分为 **`63.213854`**，相对 V149 提升 `0.000354`；冻结后 seed2100/2101/2102 三项盲测分别提升 `0.000137/0.000243/0.000349`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V152](docs/solutions/FATE-MIMO_v152.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
