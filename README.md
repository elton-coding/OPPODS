...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V147 延续 V143 的多层配对 RZF，并在 `[-7.75,-5)`、`[-0.25,2.75)`、`[8,18)` dB 为 Receiver 启用 `0.25` 的软干扰抵消强度专家。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.381521`，公平 P10 `50.694444`，总分 **`63.075398`**。三个固定 seed 的平均总分为 **`63.213054`**，相对 V143 提升 `0.002537`；冻结后 seed2098/2099/2100 三项盲测分别提升 `0.002309/0.003919/0.000030`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V147](docs/solutions/FATE-MIMO_v147.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
