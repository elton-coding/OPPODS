...
最终采用任务导向稀疏反馈、解析 Wiener 零初始化残差 Transformer、鲁棒 RZF、分层 Gray 256-QAM、正交保留导频、弱用户约束尾部向量量化，以及按 SNR 路由的 Receiver 和共享预编码参数专家。V135 在 V134 的高 SNR 分层基础上增加 `[1.50,5.75) dB → 3.0` 的中档 Receiver 协方差加载专家；其他普通区间为 `4.0`，高 SNR 两层仍为 `5.0/7.0`。

冻结版严格本地官方同口径评测（2000 样本、4000 UE 分数、seed 1176）：效率 `68.354850`，公平 P10 `50.694444`，总分 **`63.056729`**。三个固定 seed 的平均总分为 **`63.196896`**，相对 V134 提升 `0.001241`；冻结后 seed2059/2060/2061 三项盲测分别提升 `0.000699/0.003129/0.000699`，P10 均不变。以上均为本地验证结果，不等同于线上排行榜成绩。

完整设计见 [FATE-MIMO V135](docs/solutions/FATE-MIMO_v135.md)，冻结参数见 [final.yaml](configs/final.yaml)，可追溯成绩见 [冠军基准表](benchmarks/leaderboard.json)。
