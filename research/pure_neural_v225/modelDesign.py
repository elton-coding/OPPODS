...
RECEIVER_SNR_EXPERT_EDGES_DB = (-20.0, 0.0, 20.0)
RECEIVER_SNR_EXPERT_BOUNDARIES_DB = RECEIVER_SNR_EXPERT_EDGES_DB[1:-1]
NUM_RECEIVER_EXPERTS = len(RECEIVER_SNR_EXPERT_EDGES_DB) - 1
...
def _receiver_expert_indices(snr: torch.Tensor) -> torch.Tensor:
    boundaries = snr.new_tensor(RECEIVER_SNR_EXPERT_BOUNDARIES_DB)
    return torch.bucketize(snr.contiguous(), boundaries, right=True).clamp(0, NUM_RECEIVER_EXPERTS - 1)
...
class Receiver(nn.Module):
    def __init__(self):
        super().__init__()
        self.experts = nn.ModuleList([ReceiverCore() for _ in range(NUM_RECEIVER_EXPERTS)])
    ...
    def forward(
        ...
        indices = _receiver_expert_indices(snr)
