...
class TokenContext(nn.Module):
    """Learned all-subcarrier context; initially exactly the identity."""
    def __init__(self, width: int = 512, bottleneck: int = 64):
        super().__init__()
        self.norm = nn.LayerNorm(width)
        self.down = nn.Linear(width, bottleneck)
        self.mix = nn.Linear(NUM_DL_SC, NUM_DL_SC, bias=False)
        self.up = nn.Linear(bottleneck, width)
        nn.init.zeros_(self.up.weight)
        nn.init.zeros_(self.up.bias)

    def forward(self, values: torch.Tensor) -> torch.Tensor:
        hidden = nn.functional.gelu(self.down(self.norm(values)))
        hidden = self.mix(hidden.transpose(1, 2)).transpose(1, 2)
        return values + 0.1 * self.up(nn.functional.gelu(hidden))


def _load_parent_with_context(module: nn.Module, state: dict[str, torch.Tensor]) -> None:
    expected = {key for key in module.state_dict() if key.startswith("_token_context.")}
    result = module.load_state_dict(state, strict=False)
    if set(result.missing_keys) != expected or result.unexpected_keys:
        raise ValueError("invalid parent state: only new token-context parameters may be missing")
...
class TransmitterCore(nn.Module):
    def __init__(self):
        ...
        self._token_context = TokenContext(width)

    def load_parent_state(self, state: dict[str, torch.Tensor]) -> None:
        _load_parent_with_context(self, state)
    ...
    def forward(
        ...
        for index, block in enumerate(self._blocks):
            values = block(values)
            if index == 4:
                values = self._token_context(values)
