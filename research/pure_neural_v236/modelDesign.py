...
LOW_PAYLOAD_SNR_DB = -15.0
LOW_BITS_PER_RE = 4
LOW_PAYLOAD_BITS = NUM_DL_SC * LOW_BITS_PER_RE
...
def payload_lengths(snr: torch.Tensor) -> torch.Tensor:
    return torch.where(snr < LOW_PAYLOAD_SNR_DB, LOW_PAYLOAD_BITS, NUM_BITS_PER_UE).long()


def _pack_transmit_bits(bits: torch.Tensor, snr: torch.Tensor) -> torch.Tensor:
    regular = bits[:, :NUM_BITS_PER_UE].reshape(-1, NUM_DL_SC, NUM_BITS_PER_SYMBOL)
    # Untransmitted slots are the mean input, not additional source bits.
    low = torch.full_like(regular, 0.5)
    low[..., :LOW_BITS_PER_RE] = bits[:, :LOW_PAYLOAD_BITS].reshape(-1, NUM_DL_SC, LOW_BITS_PER_RE)
    return torch.where((snr < LOW_PAYLOAD_SNR_DB)[:, None, None], low, regular)


def _pack_receive_logits(logits: torch.Tensor, snr: torch.Tensor) -> torch.Tensor:
    low = logits.reshape(-1, NUM_DL_SC, NUM_BITS_PER_SYMBOL)[..., :LOW_BITS_PER_RE].reshape(-1, LOW_PAYLOAD_BITS)
    low = torch.nn.functional.pad(low, (0, NUM_BITS_PER_UE - LOW_PAYLOAD_BITS))
    return torch.where((snr < LOW_PAYLOAD_SNR_DB)[:, None], low, logits)
...
class TransmitterCore(nn.Module):
    ...
    def forward(
        ...
        for user in range(NUM_UE):
            bits = _pack_transmit_bits(bits_list[user], snr_dl[user])
...
class Receiver(nn.Module):
    ...
    def forward(
        ...
        output = _pack_receive_logits(output, snr)
        if not self.training and output.shape[0] == 1:
            return output[:, :int(payload_lengths(snr).item())]
        return output
