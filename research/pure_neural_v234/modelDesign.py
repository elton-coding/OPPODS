...
class TransmitterCore(nn.Module):
    def __init__(self):
        ...
        self._feedback_gate = nn.ModuleList([
            nn.Sequential(nn.Linear(17, 128), nn.GELU(), nn.Linear(128, 64))
            for _ in range(NUM_UE)
        ])
        for gate in self._feedback_gate:
    ...
    def load_parent_state(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        if result.unexpected_keys or any(not name.startswith("_feedback_gate.") for name in result.missing_keys):
            raise ValueError(f"invalid parent transmitter state: {result}")
    ...
    def forward(
        ...
        for user in range(NUM_UE):
            ...
            bit_features = self._bit_embed[user](bits)
            ...
            feedback_features = self._feedback_expand[user](feedback).reshape(batch, NUM_DL_SC, 16)
            user_snr = (snr_dl[user] / 20.0)[:, None, None].expand(-1, NUM_DL_SC, 1)
            scale, shift = self._feedback_gate[user](
                torch.cat([feedback_features, user_snr], dim=-1)
            ).chunk(2, dim=-1)
            features.extend([bit_features * (1.0 + scale) + shift, feedback_features])
