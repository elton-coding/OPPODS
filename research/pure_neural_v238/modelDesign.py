...
# Only training recomputation changes memory usage; inference uses plain blocks.
CHECKPOINT_TRAINING = True


def _load_widened_parent(module: nn.Module, state: dict[str, torch.Tensor]) -> None:
    target = module.state_dict()
    if set(state) != set(target) or state["_embed.weight"].shape[0] != 512:
        raise ValueError("invalid parent: expected a complete width-512 core")
    generator = torch.Generator(device="cpu").manual_seed(238)
    widened = {}
    for name, desired in target.items():
        value = state[name]
        if value.shape == desired.shape:
            widened[name] = value.clone()
        elif value.ndim == 1 and desired.shape[0] == 2 * value.shape[0]:
            widened[name] = value.repeat(2)
        elif value.ndim == 2:
            rows = desired.shape[0] // value.shape[0]
            columns = desired.shape[1] // value.shape[1]
            if rows not in (1, 2) or columns not in (1, 2):
                raise ValueError(f"invalid widening factors: {name}")
            result = value.repeat(rows, columns) / columns
            if columns == 2:
                # Antisymmetric column noise sums to zero for duplicated inputs,
                # preserving the initial function while breaking gradient symmetry.
                noise = torch.randn(
                    desired.shape[0], value.shape[1], generator=generator,
                    dtype=value.dtype, device="cpu",
                ).to(value.device)
                noise = noise * (0.01 * value.square().mean().sqrt())
                result = result + torch.cat([noise, -noise], dim=1)
            if result.shape != desired.shape:
                raise ValueError(f"invalid parent shape: {name}")
            widened[name] = result
        else:
            raise ValueError(f"invalid parent shape: {name}")
    module.load_state_dict(widened, strict=True)


def _run_blocks(blocks: nn.ModuleList, values: torch.Tensor, training: bool) -> torch.Tensor:
    for block in blocks:
        if training and torch.is_grad_enabled() and CHECKPOINT_TRAINING:
            values = checkpoint(block, values, use_reentrant=False, preserve_rng_state=False)
        else:
            values = block(values)
    return values
...
class TransmitterCore(nn.Module):
    ...
    def load_parent_state(self, state_dict: dict[str, torch.Tensor]) -> None:
        _load_widened_parent(self, state_dict)
    ...
    def forward(
        ...
        values = _run_blocks(self._blocks, values, self.training)
