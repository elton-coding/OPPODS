...
class ReceiverCore(nn.Module):
    def __init__(self):
        ...
        self._control_embed = nn.Sequential(
            nn.Linear(NUM_CTRL, 64),
            nn.GELU(),
            nn.Linear(64, width),
        )
        nn.init.zeros_(self._control_embed[-1].weight)
        nn.init.zeros_(self._control_embed[-1].bias)
        self._blocks = nn.ModuleList([ResidualMLPBlock(width) for _ in range(10)])
        self._norm = nn.LayerNorm(width)
    ...
    def forward(
        ...
        control_features = self._control_embed(2.0 * ctrl_bits - 1.0)
        values = values + control_features[:, None, :]
        for block in self._blocks:
            values = block(values)
    ...
    def _whole_min_forward(
        ...
        for expert_index, expert in enumerate(self.experts):
            ...
            if bool(selected.any().item()):
                ...
                del expert_control
        max_snr = snr_dl.amax(dim=0).clamp(-20.0, 20.0 - 1e-6)
        control_index = torch.floor((max_snr + 20.0) * (NUM_CTRL_LEVELS / 40.0)).long()
        shifts = torch.arange(NUM_CTRL, device=max_snr.device)
        control = ((control_index[:, None] >> shifts[None, :]) & 1).to(bits_list[0].dtype)
        return signal, control
