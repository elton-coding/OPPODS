...
def _control_bits(snr_dl: torch.Tensor, profile: torch.Tensor) -> torch.Tensor:
    code = ((snr_dl.sum(dim=0) + 40.0) / 5.0).floor().long().clamp(0, 15)
    powers = torch.arange(4, device=snr_dl.device)
    return torch.cat([profile[:, None], (code[:, None] >> powers) & 1], dim=1).to(snr_dl.dtype)


def _partner_snr(ctrl_bits: torch.Tensor, own_snr: torch.Tensor) -> torch.Tensor:
    powers = 2 ** torch.arange(4, device=ctrl_bits.device)
    code = (ctrl_bits[:, 1:].round().long() * powers).sum(dim=1)
    sum_snr = -40.0 + (code.to(own_snr.dtype) + 0.5) * 5.0
    return (sum_snr - own_snr).clamp(-20.0, 20.0)
...
class ReceiverCore(nn.Module):
    def __init__(self):
        ...
        self._partner_embed = nn.Linear(1, width, bias=False)
        nn.init.zeros_(self._partner_embed.weight)
        self._blocks = nn.ModuleList([ResidualMLPBlock(width) for _ in range(10)])
        self._norm = nn.LayerNorm(width)
    ...
    def load_parent_state(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        if result.unexpected_keys or any(not name.startswith("_partner_embed.") for name in result.missing_keys):
            raise ValueError(f"invalid parent receiver state: {result}")
    ...
    def forward(
        ...
        partner_feature = (_partner_snr(ctrl_bits, snr) / 20.0)[:, None]
        values = values + self._partner_embed(partner_feature)[:, None, :]
        for block in self._blocks:
            values = block(values)
    ...
    def _whole_min_forward(
        ...
        for expert_index, expert in enumerate(self.experts):
            ...
            if bool(selected.any().item()):
                ...
                del expert_control
        return signal, _control_bits(snr_dl, indices).to(bits_list[0].dtype)
...
class Receiver(nn.Module):
    ...
    def forward(
        ...
        indices = torch.round(ctrl_bits[:, 0]).long()
