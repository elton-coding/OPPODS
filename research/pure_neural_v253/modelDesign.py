...
TRANSMITTER_ROUTING = "whole_pair"
...
SNR_EXPERT_EDGES_DB = (-20.0, -15.355339059327378, -10.0, -1.213203435596423, 20.0)
SNR_EXPERT_BOUNDARIES_DB = SNR_EXPERT_EDGES_DB[1:-1]
NUM_EXPERTS = 2 * (len(SNR_EXPERT_EDGES_DB) - 1)


def _minimum_bin_indices(snr: torch.Tensor) -> torch.Tensor:
    boundaries = snr.new_tensor(SNR_EXPERT_BOUNDARIES_DB)
    return torch.bucketize(snr.contiguous(), boundaries, right=True).clamp(0, len(SNR_EXPERT_EDGES_DB) - 2)


def _pair_expert_indices(snr_dl: torch.Tensor) -> torch.Tensor:
    """Four min-SNR bands, each split at the conditional median of max-SNR."""
    minimum = snr_dl.amin(dim=0)
    maximum = snr_dl.amax(dim=0)
    upper_half = maximum >= (minimum + 20.0) / 2.0
    return 2 * _minimum_bin_indices(minimum) + upper_half.long()
...
class Transmitter(nn.Module):
    ...
    def _whole_pair_forward(
        self,
        bits_list: list[torch.Tensor],
        feedback_list: list[torch.Tensor],
        snr_dl: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        indices = _pair_expert_indices(snr_dl)
        batch = bits_list[0].shape[0]
    ...
    def forward(
        ...
        if TRANSMITTER_ROUTING == "whole_pair":
            return self._whole_pair_forward(bits_list, feedback_list, snr_dl)
