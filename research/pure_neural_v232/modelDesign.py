...
class ReceiverCore(nn.Module):
    def __init__(self):
        ...
        self._context = nn.Sequential(
            nn.Linear(2 * 2 * NUM_TX + 1, 128), nn.GELU(), nn.Linear(128, 2 * width),
        )
        nn.init.zeros_(self._context[-1].weight)
        nn.init.zeros_(self._context[-1].bias)
        self._blocks = nn.ModuleList([ResidualMLPBlock(width) for _ in range(10)])
        self._norm = nn.LayerNorm(width)
    ...
    def load_parent_state(self, state_dict: dict[str, torch.Tensor]) -> None:
        ...
        if result.unexpected_keys or any(not name.startswith("_context.") for name in result.missing_keys):
    ...
    def forward(
        ...
        scale, shift = self._context(torch.cat([h_features, noise_feature], dim=-1)).chunk(2, dim=-1)
        values = values * (1.0 + scale) + shift
        for block in self._blocks:
            values = block(values)
