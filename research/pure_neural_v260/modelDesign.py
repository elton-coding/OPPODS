...
RECEIVER_ROUTING = "parent_group_then_own_snr"
RECEIVER_LOW_OWN_THRESHOLD_DB = -10.0
RECEIVER_HIGH_OWN_BOUNDARIES_DB = (-5.0, 0.0, 5.0, 10.0, 15.0)
...
def _receiver_indices(ctrl_bits: torch.Tensor, snr: torch.Tensor) -> torch.Tensor:
    """Keep the V227 parent group, then specialize by this UE's own SNR.

    Control carries the unchanged transmitter min-SNR route, not an extra
    observation of the partner. Exactly eight independent Rx cores remain.
    """
    powers = torch.pow(2, torch.arange(NUM_CTRL, device=ctrl_bits.device))
    tx_indices = torch.sum(torch.round(ctrl_bits).long() * powers[None, :], dim=1)
    tx_indices = tx_indices.clamp(0, NUM_EXPERTS - 1)
    low_group = (snr >= RECEIVER_LOW_OWN_THRESHOLD_DB).long()
    high_group = 2 + torch.bucketize(
        snr.contiguous(), snr.new_tensor(RECEIVER_HIGH_OWN_BOUNDARIES_DB), right=True
    )
    return torch.where(tx_indices < 2, low_group, high_group)
...
class Receiver(nn.Module):
    ...
    def forward(
        ...
        indices = _receiver_indices(ctrl_bits, snr)
